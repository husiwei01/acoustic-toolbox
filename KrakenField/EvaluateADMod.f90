MODULE EvaluateADMod
  IMPLICIT NONE
  INTEGER, PARAMETER, PRIVATE :: MaxNfreq = 1000
...
CONTAINS
  ...
  ! Initialization
  rProf( NProf + 1 ) = 1e20   ! HUGE( rProf( NProf ) ), but huge produce an underflow later
  iProf              = 1
  ...
  Ranges: DO ir = 1, Nr
     ...
     DO WHILE ( r( ir ) > 1000.0 * rProf( iProf + 1 ) )
        CALL NewSegment( FileRoot, r, ir, rProf, iProf, ifreq, NProf, kL, kR, phiL, phiR, M, MaxM, rd, Nrd, sumk, sumkinv )
     END DO
...
SUBROUTINE NewSegment( FileRoot, r, ir, rProf, iProf, ifreq, NProf, kL, kR, phiL, phiR, M, MaxM, rd, Nrd, sumk, sumkinv )

  ! Treats the crossing into a new range segment

  USE ReadModes
  IMPLICIT NONE
  INTEGER,            INTENT( IN    ) :: ir, MaxM, NProf, Nrd                  ! range index, max # modes, # profiles, # receiver depths
  ...
  INTEGER,            INTENT( INOUT ) :: iProf, M                              ! profile index, # modes
