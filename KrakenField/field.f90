PROGRAM FIELD
  ...
  CASE DEFAULT
     CALL ERROUT( 'FIELD', 'Unknown value for Option( 1 : 1 ); should be X or R' )
  END SELECT
  ...
  CALL ReadVector( NProf, RProf, 'Profile ranges, RProf', 'km' )   ! Read profile ranges (where a new mode set takes effect)
  ...
  ELSE
     ...
     CASE DEFAULT
        CALL ERROUT( 'FIELD', 'Unknown option for type of calc; should be Adiabatic or Coupled modes' )
     END SELECT
  END IF

  ! EvaluateAD/EvaluateCM need a profile at zero range
  IF ( rProf( 1 ) /= 0.0 ) CALL ERROUT( 'FIELD', 'The first profile must be at 0 km' )
